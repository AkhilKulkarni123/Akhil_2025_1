...
class GameLevelDesert {
  constructor(path) {
    ...
    // Add event listener for the 'e' key to trigger quiz interaction
  ...
  setupKeyListeners() {
    ...
    const keyPressHandler = (event) => {
      ...
      // Handle interaction key (e) for quizzes
      if (event.keyCode === 69) { // 'E' key
        this.objects.forEach(obj => {
          if (obj.class === Npc) {
            const npc = obj.data;
            // Check if player is near the NPC and open the quiz if so
            if (this.isPlayerNearNpc(player, npc)) {
              this.showQuiz(npc.quiz);
            }
          }
        });
      }
  ...
  showQuiz(quizData) {
    alert(`Welcome to the ${quizData.title}!`);
    quizData.questions.forEach((question, index) => {
      alert(`${index + 1}. ${question}`);
    });
  }
}
