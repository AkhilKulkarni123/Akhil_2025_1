...
class GameLevelDesert {
  constructor(path) {
    ...
    // Collision tally logic
    this.collisionCount = 0;
    this.tallyDisplay = document.createElement('div');
    this.tallyDisplay.style.position = 'absolute';
    this.tallyDisplay.style.top = '10px';
    this.tallyDisplay.style.left = '10px';
    this.tallyDisplay.style.fontSize = '20px';
    this.tallyDisplay.style.color = 'white';
    document.body.appendChild(this.tallyDisplay);
    ...
    this.checkCollisions = () => {
        this.objects.forEach(obj => {
            if (obj.class === Npc) {
                const npc = obj.data;
                // Assuming checkCollision is a function to check if player collides with NPC
                if (checkCollision(sprite_data_chillguy, npc)) {
                    this.collisionCount++;
                    this.updateCollisionDisplay();
                }
            }
    ...
    this.updateCollisionDisplay = () => {
        this.tallyDisplay.textContent = `Collisions: ${this.collisionCount}`;
    };

    // ESC key to proceed logic
    window.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            if (this.collisionCount < 15) {
                alert("You are not done yet buddy!");
            } else {
                // Logic to proceed to the next level (water)
                // For example:
                // window.location.href = "next_level_water.html";
